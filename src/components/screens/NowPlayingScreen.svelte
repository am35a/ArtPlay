...
<section class="now-playing d_l_align-content--center">
  ...
  <section class="visual d_l_margin-top--auto d_l_margin-inline--auto">
    <e-group class="element--group d_l_display--inline-grid d_l_grid-template-columns--2" role="group">
      <button type="button" disabled={state.visualizerMode === 'spectrum'} onclick={() => dispatch('mode', { mode: 'spectrum' })}>
      ...
      <button type="button" disabled={state.visualizerMode === 'oscilloscope'} onclick={() => dispatch('mode', { mode: 'oscilloscope' })}>
  ...
  <section class="playback-controls">
    <div class="playback-range d_l_display--grid d_l_position--relative">
      ...
      <input
        ...
        step="0.1"
        value={current}
        oninput={(event) => dispatch('seek', { value: Number(event.currentTarget.value) })}
      />
    </div>

    <div class="control-row d_l_justify-items--center d_l_font-size--l3">
      <button class="d_l_padding-inline--s3 d_l_background-color--transparent" type="button" onclick={() => dispatch('prev')}>
      ...
      <button class="d_l_padding-inline--s3 d_l_background-color--transparent" type="button" onclick={() => dispatch('rewind')}>
      ...
      <button type="button" class="prime d_l_padding-inline--s3 d_l_border-radius--l2 d_l_font-size--l2" onclick={() => dispatch('toggle')}>
        {#if state.isPlaying}
      ...
      <button class="d_l_padding-inline--s3 d_l_background-color--transparent" type="button" onclick={() => dispatch('forward')}>
      ...
      <button class="d_l_padding-inline--s3 d_l_background-color--transparent" type="button" onclick={() => dispatch('next')}>
    ...
    <div class="mode-row">
      <button type="button" onclick={() => dispatch('shuffle')}>
        Шаффл:
        {#if state.shuffle}
